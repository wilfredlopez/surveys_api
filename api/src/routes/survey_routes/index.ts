...
import SurveyQuestionDB from '../../db/SurveyQuestion'
...
import { Survey } from '../../interfaces/surveyInterfaces'
...
class SurveyRoutes {
  ...
  updateQuestion: RequestHandler<{ id: string; questionId: string }> = async (
    ...
    try {
      ...
      const body = req.body as Partial<SurveyQuestion>
    ...
    try {
      ...
      if (valuesToUpdate.questions) {
        ...
        if (Array.isArray(valuesToUpdate.questions)) {
          const nonExistingQuestions = (valuesToUpdate.questions as SurveyQuestion[]).filter(
            q => q._id === ''
          )
          const ExistingQuestions = (valuesToUpdate.questions as SurveyQuestion[]).filter(
            q => q._id !== ''
          )
          const raw_questions = BaseSurveyQuestion.transformQuestions(
  ...
  addSurveyResponse: RequestHandler = async (req, res) => {
    try {
      ...
      if (BaseSurveyQuestion.isValidAnswerInput(answers) === false) {
        res.status(400).json({
      ...
      await BaseSurveyQuestion.addAnswers(SurveyQuestionDB, answers)
      await survey.save()
      res.json(survey)
    } catch (error) {
      console.error(error)
